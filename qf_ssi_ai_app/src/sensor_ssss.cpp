...
int  sensor_ssss_acquisition_buffer_ready()
{
    ...
    int16_t *p_all_sensor_data = (int16_t *)p_dest;

    *p_all_sensor_data++ = accel_data.x;
    *p_all_sensor_data++ = accel_data.y;
    *p_all_sensor_data++ = accel_data.z;

    /* Read data from other sensors */
    int16_t adc_data = qorc_ssi_adc.getSingleEnded(3); /* Read ADC data from ADS1015 */

    *p_all_sensor_data++ = adc_data;
